...
import { obtenerIdsPorPalabrasClave } from '../utils/index.js'
...
router.post("/anuncio", async (req, res) => {
  ...
  try {
    ...
    const usuariosIds = await obtenerIdsPorPalabrasClave(data.palabra_clave);

    for (const usuario of usuariosIds) {
      await dbClient.query(
        "INSERT INTO notificaciones (usuario_id, anuncio_id) VALUES ($1, $2)",
        [usuario.id, anuncioId]
      );
    }
